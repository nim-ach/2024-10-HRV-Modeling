...
The fitting process utilized five Markov Chain Monte Carlo (MCMC) chains, each consisting of 10,000 iterations with a burn-in period of 5,000 iterations, resulting in a total of 25,000 post-warmup samples. 
...
Firstly, each subject's RRi data $\text{RRi}_{i,t}$ was standardize against his own mean $\bar{\text{RRi}_i}$ and standard deviation $\sigma_{\text{RRi}_i}$ to improve convergence and exploration of the posterior distribution. The standardized RRi data $y_{i,t}$ for each time point $t$ was computed as:
...
[**Figure 1**]{#fig-prior-sim}. Prior predicted RRi response to exercise, illustrating uncertainty in model parameters from the prior distribution. Shaded areas represent 95%, 80%, and 60% quantile CI.
...
For each subject $i$, the parameter vector $\theta_i = (\alpha_i, \beta_i, c_i, \lambda_i, \phi_i, \tau_i, \delta_i)^\top$ is modeled as:

$$
\theta_{i,j} \sim \mathcal{N}(\mu_{j} + b_{i,j}, \sigma_j^2)
$$

where $\mu_{j}$ is the group-level mean for parameter $j$, $b_{i,j}$ represents the subject-level random effect for subject $i$ on parameter $j$, and $\sigma_j^2$ is the residual variance for parameter $j$.

The subject-level effects $b_{i,j}$ are assumed to follow a multivariate normal distribution:

$$
\mathbf{b}_i = (b_{i, \alpha}, b_{i, \beta}, b_{i, c}, b_{i, \lambda}, b_{i, \phi}, b_{i, \tau}, b_{i, \delta})^\top \sim \mathcal{N}(\mathbf{0}, \Sigma_b)
$$

where $\Sigma_b$ is the covariance matrix that captures correlations between the random effects for different parameters across subjects.

This hierarchical structure enables us to capture individual variability through subject-level random effects while also estimating group-level effects across all parameters, thus providing estimates into both subject-level and population-level model parameters.
